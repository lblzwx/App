...
+(NSString *)getRidofP:(NSString *)count{
    NSString *str1 = [count stringByReplacingOccurrencesOfString:@"</p>" withString:@""];
    str1 = [str1 stringByReplacingOccurrencesOfString:@"<p>" withString:@""];
    return str1;
}

+(NSString *)getRidofHttp:(NSString *)content{
    while (1) {
        NSString *p1 = @"<a href=\"(.*?)\" target=\"_blank\">";
        NSString *html = [content firstMatchWithPattern:p1];
        content = [content stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"<a href=\"%@\" target=\"_blank\">",html] withString:@""];
        content = [content stringByReplacingOccurrencesOfString:@"</a>" withString:@""];
        if (html == nil) {
            break;
        }
    }
